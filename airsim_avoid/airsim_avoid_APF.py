...
if __name__ == "__main__":
    ...
    client.takeoffAsync(vehicle_name='SimpleFlight')
    # client.moveToZAsync(-3, 1, vehicle_name='SimpleFlight').join()

    # 手动设置航路点
    points = [airsim.Vector3r(2, 0, -3),
              airsim.Vector3r(3, -4, -3),
              airsim.Vector3r(6, 5, -3),
    ...
    move_by_path_and_avoid_APF(client, points, K_track=[1.5, 6, 1], delta=5, K_avoid=[3, 60], Q_search=8, epsilon=1,
                               Ul=[2, 3], dt=0.3, vehicle_name='SimpleFlight')

    client.landAsync(vehicle_name='SimpleFlight').join()
    client.armDisarm(False, vehicle_name='SimpleFlight')
    client.enableApiControl(False, vehicle_name='SimpleFlight')
